    ...
    <>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading && !orders.length ? <tr><td colSpan="7" className="px-4 py-6 text-center text-slate-500">No orders found.</td></tr> : null}
            {orders.map((order) => (
              <tr key={order.id}>
                ...
                <td className="px-4 py-3"><span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700">{getOrderStatusLabel(order.order_status)}</span></td>
                ...
                <td className="px-4 py-3 text-right"><Link to={`/admin/orders/${order.id}`} className="font-semibold text-emerald-700">View</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {meta?.last_page > 1 && (
        <div className="mt-4 flex items-center justify-end gap-2">
          <button disabled={params.page <= 1} onClick={() => updateParams({ ...params, page: params.page - 1 })} className="rounded-md border border-slate-300 px-3 py-1 text-sm disabled:opacity-50">Previous</button>
          <span className="text-sm text-slate-600">{params.page} / {meta.last_page}</span>
          <button disabled={params.page >= meta.last_page} onClick={() => updateParams({ ...params, page: params.page + 1 })} className="rounded-md border border-slate-300 px-3 py-1 text-sm disabled:opacity-50">Next</button>
        </div>
      )}
    </>
  )
