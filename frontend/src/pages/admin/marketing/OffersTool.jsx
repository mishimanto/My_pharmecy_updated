...
export default function OffersTool() {
  ...
  const empty = { label: '', label_bn: '', title: '', title_bn: '', body: '', body_bn: '', button_label: 'Browse medicines', button_label_bn: 'ওষুধ দেখুন', link_url: '/products', image_url: '', sort_order: 0, show_in_nav: false, starts_at: '', ends_at: '', status: 'active' }
  ...
  const stats = useMemo(() => [['Offers', items.length], ['Active', items.filter((item) => item.status === 'active').length], ['Placement', 'Homepage']], [items])
  ...
  const reset = () => { setForm(empty); setEditingId(null); setImageFile(null); setPreview('') }
  const edit = (item) => { setEditingId(item.id); setForm({ ...empty, ...item, show_in_nav: Boolean(item.show_in_nav), image_url: item.image_path ? '' : item.image_url || '', starts_at: toDateTimeLocal(item.starts_at), ends_at: toDateTimeLocal(item.ends_at) }); setPreview(item.image_src || item.image_url || ''); setImageFile(null) }
  ...
  const remove = async (item) => {
    ...
    await adminApi.remove('offers', item.id).then(() => { toast.success('Offer deleted.'); load() }).catch(() => toast.error('Unable to delete offer.'))
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
        <MarketingList title="Storefront offers" items={items} emptyText="No offers yet." onEdit={edit} onRemove={remove} renderMeta={(item) => [item.label || 'No label', item.show_in_nav ? 'Top nav' : null].filter(Boolean).join(' / ')} renderTitle={(item) => item.title} image />
