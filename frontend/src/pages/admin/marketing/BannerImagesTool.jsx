...
import { FiCheckCircle, FiEdit, FiImage, FiMapPin, FiSave, FiTrash2, FiUploadCloud, FiX } from 'react-icons/fi'
...
export default function BannerImagesTool() {
  ...
  const stats = useMemo(() => ([
    { label: 'Banners', value: items.length, variant: 'sky', icon: FiImage },
    { label: 'Active', value: items.filter((item) => item.status === 'active').length, variant: 'emerald', icon: FiCheckCircle },
    { label: 'Placement', value: 'Homepage', variant: 'amber', icon: FiMapPin },
  ]), [items])
  const placementOptions = useMemo(() => [...new Set(items.map((item) => item.placement || 'homepage'))], [items])
  const filteredItems = useMemo(() => {
    const query = search.trim().toLowerCase()

    return items.filter((item) => {
      const placement = item.placement || 'homepage'
      ...
      const matchesPlacement = !filters.placement || placement === filters.placement

      return matchesSearch && matchesStatus && matchesPlacement
    })
  }, [filters.placement, filters.status, items, search])
  const hasActiveFilters = Boolean(search || filters.status || filters.placement)
  const filterOptions = [
    {
      key: 'status',
    ...
    },
    {
      ...
      onChange: (value) => setFilters((current) => ({ ...current, placement: value })),
      options: [
        { value: '', label: 'All Placements' },
        ...placementOptions.map((placement) => ({ value: placement, label: formatLabel(placement) })),
      ],
    },
  ]
  ...
  return (
    <div className="space-y-5">
      ...
      <AdminFilterBar
        ...
        searchPlaceholder="Search by title, label, or placement"
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <div className="divide-y divide-slate-100">
              ...
              <div key={item.id} className="grid gap-4 px-5 py-4 lg:grid-cols-[180px_minmax(0,1fr)_auto] lg:items-center">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-500">#{item.sort_order || 0}</span>
            ...
            {filteredItems.length === 0 ? <div className="px-5 py-10 text-center text-sm text-slate-500">No banner images found.</div> : null}
          </div>
        </section>
