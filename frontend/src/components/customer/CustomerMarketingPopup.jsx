    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/50 px-4 backdrop-blur-sm">
      <article className="relative h-[560px] w-full max-w-2xl overflow-hidden bg-slate-900 shadow-[0_30px_90px_-45px_rgba(15,23,42,0.75)] sm:h-[620px]">
        <button
          type="button"
          onClick={close}
          className="absolute rounded-full right-3 top-3 z-10 inline-flex h-9 w-9 items-center justify-center text-slate-700 shadow-sm transition hover:text-slate-950"
        ...
        <div className="absolute inset-0 bg-linear-to-t from-slate-950 via-slate-950/35 to-slate-950/5" />
        ...
        <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-8">
          ...
          <h2 className="mt-4 max-w-xl text-3xl font-bold leading-tight sm:text-4xl">{popup.title}</h2>
          {popup.body ? <p className="mt-3 max-w-lg text-sm leading-6 text-slate-100 sm:text-base">{popup.body}</p> : null}

          <div className="mt-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            <span className="inline-flex items-center justify-center gap-2 text-sm font-bold text-[#a2b6b9]">
            ...
            <Link
              to={popup.linkUrl}
              onClick={close}
              className="inline-flex items-center justify-center gap-1 underline text-sm font-semibold text-white transition hover:text-bold"
            >
